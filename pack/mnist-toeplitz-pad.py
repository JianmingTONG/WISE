...
# run  := python3 mnist-toeplitz-pad.py
# dir  := .
# kid  :=
...
def pack():
    ...
    def conv1(kernel, bias, scale=1.0):
        lt = pack_conv2d_toeplitz(
            in_order=Order(batchsize=(64, 1, 1), C=1, H=32, W=32, H_real=28, W_real=28, tilesize=(2, 2)),
            out_order=Order(batchsize=(1, 64, 1), C=32, H=32, W=32, H_real=28, W_real=28, tilesize=(2, 2)),
            stride=1,
            kernel=kernel,
            bn_b=bias,
            out_scale=scale,
    ...
    def act1(a0, a1, a2):
        order = Order(batchsize=(1, 64, 1), C=32, H=32, W=32, H_real=28, W_real=28, tilesize=(2, 2))
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act1", Layer.HERPN, hp)

    def pool1(scale=1.0):
        lt = pack_avgpool2d(
            in_order=Order(batchsize=(1, 64, 1), C=32, H=32, W=32, H_real=28, W_real=28, tilesize=(2, 2)),
            out_order=Order(batchsize=(1, 64, 1), C=32, H=16, W=16, H_real=14, W_real=14),
            size=2,
            stride=2,
            padding=0,
            out_scale=scale,
    ...
    def conv2(kernel, bias, scale=1.0):
        lt = pack_conv2d_toeplitz(
            in_order=Order(batchsize=(1, 64, 1), C=32, H=16, W=16, H_real=14, W_real=14),
            out_order=Order(batchsize=(1, 64, 1), C=64, H=16, W=16, H_real=14, W_real=14),
            stride=1,
            kernel=kernel,
            bn_b=bias,
            out_scale=scale,
        )
        dump("conv2", Layer.LINEARTRANSFORM, lt)

    def act2(a0, a1, a2):
        order = Order(batchsize=(1, 64, 1), C=64, H=16, W=16, H_real=14, W_real=14)
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act2", Layer.HERPN, hp)

    def pool2(scale=1.0):
        lt = pack_avgpool2d(
            in_order=Order(batchsize=(1, 64, 1), C=64, H=16, W=16, H_real=14, W_real=14),
            out_order=Order(batchsize=(1, 64, 1), C=64, H=8, W=8, H_real=7, W_real=7, gap=(2, 2)),
            size=2,
            stride=2,
            padding=0,
            out_scale=scale,
    ...
    def fc1(weight, bias, scale=1.0):
        ...
        lt = pack_fc(
            in_order=Order(batchsize=(1, 64, 1), C=64, H=8, W=8, H_real=7, W_real=7, gap=(2, 2)),
            out_order=Order(batchsize=(64, 1, 1), C=256, C_real=196, H=1, W=1),
            weight=weight,
            bias=bias,
            out_scale=scale,
        )
    ...
    def act3(a0, a1, a2):
        order = Order(batchsize=(64, 1, 1), C=256, C_real=196, H=1, W=1)
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act3", Layer.HERPN, hp)

    def fc2(weight, bias, scale=1.0):
        lt = pack_fc(
            in_order=Order(batchsize=(64, 1, 1), C=256, C_real=196, H=1, W=1),
            out_order=Order(batchsize=(64, 1, 1), C=10, H=1, W=1),
            weight=weight,
            bias=bias,
            out_scale=scale,
        )
