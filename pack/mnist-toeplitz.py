...
def pack():
    ...
    def conv1(kernel, bias, scale=1.0):
        lt = pack_conv2d_toeplitz(
            in_order=Order(batchsize=(64, 1, 1), C=1, H=28, W=28, tilesize=(2, 2)),
            out_order=Order(batchsize=(1, 64, 1), C=32, H=28, W=28, tilesize=(2, 2)),
            stride=1,
            kernel=kernel,
            bn_b=bias,
            out_scale=scale,
    ...
    def act1(a0, a1, a2):
        order = Order(batchsize=(1, 64, 1), C=32, H=28, W=28, tilesize=(2, 2))
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act1", Layer.HERPN, hp)

    def pool1(scale=1.0):
        lt = pack_avgpool2d(
            in_order=Order(batchsize=(1, 64, 1), C=32, H=28, W=28, tilesize=(2, 2)),
            out_order=Order(batchsize=(1, 64, 1), C=32, H=14, W=14),
            size=2,
            stride=2,
            padding=0,
            out_scale=scale,
    ...
    def conv2(kernel, bias, scale=1.0):
        lt = pack_conv2d_toeplitz(
            in_order=Order(batchsize=(1, 64, 1), C=32, H=14, W=14),
            out_order=Order(batchsize=(1, 64, 1), C=64, H=14, W=14),
            stride=1,
            kernel=kernel,
            bn_b=bias,
            out_scale=scale,
    ...
    def act2(a0, a1, a2):
        order = Order(batchsize=(1, 64, 1), C=64, H=14, W=14)
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act2", Layer.HERPN, hp)

    def pool2(scale=1.0):
        lt = pack_avgpool2d(
            in_order=Order(batchsize=(1, 64, 1), C=64, H=14, W=14),
            out_order=Order(batchsize=(1, 64, 1), C=64, H=7, W=7, gap=(2, 2)),
            size=2,
            stride=2,
            padding=0,
            out_scale=scale,
    ...
    def fc1(weight, bias, scale=1.0):
        lt = pack_fc(
            in_order=Order(batchsize=(1, 64, 1), C=64, H=7, W=7, gap=(2, 2)),
            out_order=Order(batchsize=(64, 1, 1), C=196, H=1, W=1),
            weight=weight,
            bias=bias,
            out_scale=scale,
        )
    ...
    def act3(a0, a1, a2):
        order = Order(batchsize=(64, 1, 1), C=196, H=1, W=1)
        hp = pack_herpn(order, a0, a1 / np.sqrt(a2), a2)
        dump("act3", Layer.HERPN, hp)

    def fc2(weight, bias, scale=1.0):
        lt = pack_fc(
            in_order=Order(batchsize=(64, 1, 1), C=196, H=1, W=1),
