#!/home/jyh/project/winograd/winograd-venv/bin/python3
# run  := python3 resnet34_winograd_silu_avgpool_pack.py
# dir  := .
# kid  :=

from logging import NOTSET
import torch
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "..", "src"))
from sagittarius.core import (
    Batchnorm2d,
    Layer,
    Order,
    pack_avgpool2d,
    pack_conv2d_toeplitz,
    pack_conv2d_winograd,
    num_diags,
    Cipher,
    Order
)
import pickle
import torchvision
from torchvision.datasets import ImageFolder
from torch.utils.data import DataLoader
import numpy as np

def resnet34():
    model = torch.jit.load("../resnet34-clear/models/resnet34_silu_avgpool_deploy.pt", map_location="cpu")
    model.eval()
    sd = model.state_dict()

    base_dir = "/home/jyh/project/winograd/data/resnet34-winograd-silu-avgpool"

    def load_kernel(prefix):
        prefix = f"backbone.{prefix}"
        return sd[f"{prefix}.weight"]

    def load_bn(prefix):
        prefix = f"backbone.{prefix}"
        def resolve_submodule(root, path: str):
            m = root
            for name in path.split('.'):
                m = getattr(m, name)
            return m

        bn = resolve_submodule(model, prefix)
        return Batchnorm2d(
            mean=bn.running_mean.detach().numpy(),
            var=bn.running_var.detach().numpy(),
            gamma=bn.weight.detach().numpy(),
            beta=bn.bias.detach().numpy(),
            eps=bn.eps
        )

    N = 32768

    def conv1():
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=3, H=256, W=256, H_real=224, W_real=224, Ht=2, Wt=2),
            out_order=Order(C=64, H=128, W=128, H_real=112, W_real=112, Ht=2, Wt=2, gap=2),
            stride=2,
            kernel=load_kernel("conv1"),
            batchnorm=load_bn("bn1")
        )
        with open(f"{base_dir}/raw/conv1.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/conv1.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def pool1():
        lt = pack_avgpool2d(
            in_order=Order(C=64, H=128, W=128, H_real=112, W_real=112, Ht=2, Wt=2, gap=2),
            out_order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            size=3,
            stride=2
        )
        with open(f"{base_dir}/raw/pool1.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/pool1.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer1_0_conv1():
        prefix, cid = "layer1.0", 1
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        print("Start writing")
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer1_0_conv2():
        prefix, cid = "layer1.0", 2
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer1_1_conv1():
        prefix, cid = "layer1.1", 1
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer1_1_conv2():
        prefix, cid = "layer1.1", 2
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer1_2_conv1():
        prefix, cid = "layer1.2", 1
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer1_2_conv2():
        prefix, cid = "layer1.2", 2
        wino = pack_conv2d_winograd(
            order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)


    def layer2_0_downsample():
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            out_order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            stride=2,
            kernel=load_kernel("layer2.0.downsample.0"),
            batchnorm=load_bn("layer2.0.downsample.1")
        )
        with open(f"{base_dir}/raw/layer2_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/layer2_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer2_0_conv1():
        prefix, cid = "layer2.0", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=64, H=64, W=64, H_real=56, W_real=56, Ht=2, Wt=2, gap=4),
            out_order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            stride=2,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer2_0_conv2():
        prefix, cid = "layer2.0", 2
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_1_conv1():
        prefix, cid = "layer2.1", 1
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_1_conv2():
        prefix, cid = "layer2.1", 2
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_2_conv1():
        prefix, cid = "layer2.2", 1
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_2_conv2():
        prefix, cid = "layer2.2", 2
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_3_conv1():
        prefix, cid = "layer2.3", 1
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    def layer2_3_conv2():
        prefix, cid = "layer2.3", 2
        wino = pack_conv2d_winograd(
            order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.WINOGRAD, wino.pack(N)), f)

    # NOTE: layer 3
    def layer3_0_downsample():
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=2,
            kernel=load_kernel("layer3.0.downsample.0"),
            batchnorm=load_bn("layer3.0.downsample.1")
        )
        with open(f"{base_dir}/raw/layer3_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/layer3_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_0_conv1():
        prefix, cid = "layer3.0", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=2,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_0_conv2():
        prefix, cid = "layer3.0", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_1_conv1():
        prefix, cid = "layer3.1", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_1_conv2():
        prefix, cid = "layer3.1", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_2_conv1():
        prefix, cid = "layer3.2", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_2_conv2():
        prefix, cid = "layer3.2", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_3_conv1():
        prefix, cid = "layer3.3", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_3_conv2():
        prefix, cid = "layer3.3", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_4_conv1():
        prefix, cid = "layer3.4", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_4_conv2():
        prefix, cid = "layer3.4", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_5_conv1():
        prefix, cid = "layer3.5", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer3_5_conv2():
        prefix, cid = "layer3.5", 2
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=1,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    # NOTE: layer 3
    def layer4_0_downsample():
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=8),
            out_order=Order(C=512, H=8, W=8, H_real=7, W_real=7, Ht=2, Wt=2, gap=16),
            stride=2,
            kernel=load_kernel("layer3.0.downsample.0"),
            batchnorm=load_bn("layer3.0.downsample.1")
        )
        with open(f"{base_dir}/raw/layer3_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/layer3_0_downsample.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)

    def layer4_0_conv1():
        prefix, cid = "layer4.0", 1
        lt = pack_conv2d_toeplitz(
            in_order=Order(C=128, H=32, W=32, H_real=28, W_real=28, Ht=2, Wt=2, gap=8),
            out_order=Order(C=256, H=16, W=16, H_real=14, W_real=14, Ht=2, Wt=2, gap=16),
            stride=2,
            kernel=load_kernel(f"{prefix}.conv{cid}"),
            batchnorm=load_bn(f"{prefix}.bn{cid}")
        )
        with open(f"{base_dir}/raw/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt), f)
        with open(f"{base_dir}/ready/{prefix.replace('.', '_')}_conv{cid}.pkl", "wb") as f:
            pickle.dump((Layer.LINEARTRANSFORM, lt.pack(N)), f)



    # conv1()
    # pool1()

    # layer1_0_conv1()
    # layer1_0_conv2()

    # layer1_1_conv1()
    # layer1_1_conv2()

    # layer1_2_conv1()
    # layer1_2_conv2()

    # layer2_0_downsample()
    # layer2_0_conv1()
    # layer2_0_conv2()

    # layer2_1_conv1()
    # layer2_1_conv2()

    # layer2_2_conv1()
    # layer2_2_conv2()

    # layer2_3_conv1()
    # layer2_3_conv2()

    def meta_process():
        def fetch(batchsize=1, seed=None):
            root="/home/jyh/project/winograd/resnet34-clear/dataset"
            weights = torchvision.models.ResNet34_Weights.IMAGENET1K_V1
            preprocess = weights.transforms()
            ds = ImageFolder(f"{root}/val", transform=preprocess)
            generator = None
            if seed is not None:
                generator = torch.Generator()
                generator.manual_seed(seed)
            loader = DataLoader(
                ds,
                batch_size=batchsize,
                shuffle=True,
                drop_last=True,
                num_workers=4,
                pin_memory=torch.cuda.is_available(),
                generator=generator,
            )
            images, labels = next(iter(loader))
            return images, labels

        input, label = fetch(1, seed=42)

        files = ["conv1.pkl", "layer1_0_conv1.pkl", "layer1_0_conv2.pkl", "layer1_1_conv1.pkl", "layer1_1_conv2.pkl", "layer1_2_conv1.pkl", "layer1_2_conv2.pkl", "layer2_0_conv1.pkl", "layer2_0_conv2.pkl", "layer2_0_downsample.pkl", "layer2_1_conv1.pkl", "layer2_1_conv2.pkl", "layer2_2_conv1.pkl", "layer2_2_conv2.pkl", "layer2_3_conv1.pkl", "layer2_3_conv2.pkl", "pool1.pkl"]

        input = input.numpy()

        def convert(x):
            padded = 256 - 224
            x = np.pad(x, ((0, 0),(0, 0),(0, padded),(0, padded)), 'constant')
            c = Cipher(x.flatten(), order=Order(C=3, H=256, W=256, H_real=224, W_real=224))
            c.repack(Ht=2, Wt=2, gap=1, depth=0)
            return c.v.flatten()

        global_rots = set()
        for file in files:
            with open(f"/home/jyh/project/winograd/data/resnet34-winograd-silu-avgpool/ready/{file}", "rb") as f:
                tp, obj = pickle.load(f)
            if tp == Layer.LINEARTRANSFORM:
                global_rots.update(obj.global_rots)
            elif tp == Layer.WINOGRAD:
                s = set([int(i) for i in list(obj.i2c_offset.flatten()) if i != 0])
                global_rots.update(s)
                global_rots.update(obj.global_rots)

        meta = {
            "input": convert(input),
            "rots": sorted(list(global_rots)),
            "label": label
        }

        with open("/home/jyh/project/winograd/data/resnet34-winograd-silu-avgpool/ready/meta.pkl", "wb") as f:
            pickle.dump(meta, f)

    meta_process()


if __name__ == "__main__":
    resnet34()
